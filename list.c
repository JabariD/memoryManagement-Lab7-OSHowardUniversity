...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
  node_t* newBlk = node_alloc(newblk);
  newBlk->next = NULL;
  ...
  // head case
  if (newBlk->blk->start < l->head->blk->start) {
    newBlk->next = l->head;
    l->head = newBlk;
    added = 1;
  } else {
    ...
    for (ptr = l->head; ptr->next != NULL; ptr = ptr->next) {
      if (newBlk->blk->start < ptr->next->blk->start) {
        newBlk->next = ptr->next;
        ptr->next = newBlk;
        added = 1;
        break;
      }
...
// {0,200} {201, 400} {405, 1000}
void list_coalese_nodes(list_t *l) { 
  if (list_length(l) < 2) return;

  node_t* curr = l->head;
  node_t* next = l->head->next;
  while (next != NULL) {
    if (curr->blk->end + 1 == next->blk->start) {
      curr->blk->end = next->blk->end;
      next = next->next;
      list_remove_at_index(l, list_get_index_of(l, curr->next->blk));
...
bool list_is_in_by_pid(list_t *l, int pid){ 
  ...
  * Look at list_is_in_by_size() */
  ...
  for (ptr = l->head; ptr != NULL; ptr = ptr->next) {
    if (comparePid(pid, ptr->blk)) return true;
  }
  return false;
}
