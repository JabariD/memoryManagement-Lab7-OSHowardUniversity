...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  node_t* ptr;
  int processCanBeAllocated = 0;
  int computedStartValue;
  int originalEnd;
  for (ptr = freelist->head; ptr != NULL; ptr = ptr->next) {
    if (ptr->blk->end - ptr->blk->start >= blocksize) {
        ptr->blk->pid = pid;

        originalEnd = ptr->blk->end;
        ptr->blk->end = ptr->blk->start + blocksize - 1;

        // Fragment Calculation
        computedStartValue = ptr->blk->end + 1;

        // Add to allocated memory
        list_add_ascending_by_address(alloclist, ptr->blk); 

        // Remove this from memory list
        list_remove_at_index(freelist, list_get_index_of(freelist, ptr->blk));
        processCanBeAllocated = 1;
        break;
    }
  }
  if (!processCanBeAllocated) {
    printf("Error: Memory Allocation %d blocks\n", blocksize);
    return;
  }

  block_t* fragmentBlock = malloc(sizeof(block_t));
  fragmentBlock->pid = 0;
  fragmentBlock->start = computedStartValue;
  fragmentBlock->end = originalEnd;

  if (policy == 1) {
    list_add_to_back(freelist, fragmentBlock); // FIFO
  } else if (policy == 2) {
    list_add_ascending_by_blocksize(freelist, fragmentBlock); // BEST FIT
  } else {
    list_add_descending_by_blocksize(freelist, fragmentBlock); // WORST FIT
  }
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
   ...
   if (list_is_in_by_pid(alloclist, pid)) {
       int indexOfMemoryBlock = list_get_index_of_by_Pid(alloclist, pid);
       block_t* block = list_get_elem_at(alloclist, indexOfMemoryBlock);
       block->pid = 0;
       
       if (policy == 1) {
           list_add_to_back(freelist, block); // FIFO
       } else if (policy == 2) {
           list_add_ascending_by_blocksize(freelist, block); // BEST FIT
       } else {
           list_add_descending_by_blocksize(freelist, block); // WORST FIT
       }

       list_remove_at_index(alloclist, indexOfMemoryBlock);
   } else {
